...
const MonthCalendar = memo(function MonthCalendar({ currentDate, journalEntries = {}, favorites = [], onDateSelect }) {
  ...
  // Memoize favorite dates for O(1) lookup
  const favoriteDates = useMemo(() => {
    const dates = new Set();
    favorites.forEach(fav => {
      // Favorites store savedAt timestamp, convert to date string for comparison
  ...
  const hasJournalEntry = (date) => {
    // Format date as YYYY-MM-DD to match App.jsx storage format
